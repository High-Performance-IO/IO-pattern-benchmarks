...
int main(int argc, char **argv) {
    ...
    for (auto i = 0; i < file_count; i++) {
        ...
        std::cout << "Reading from file: " << input_file_name;

        std::chrono::steady_clock::time_point test_start = std::chrono::steady_clock::now();
        std::ifstream output_file(input_file_name, std::ios::binary);

        int read_operations = file_size / window_size;
        int extra_read_size = file_size % window_size;
        for (int operation_id = 0; operation_id < read_operations; operation_id++) {
